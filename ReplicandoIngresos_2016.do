...
********************************************************************************
*************		REPLICANDO INDICADORES INE 2016                *************
********************************************************************************

use "D:\Encuestas Hogares\Encuesta de Hogares 2016\eh_2016_persona.dta"
global year 2016
...
capture drop aux2
gen aux2 = 120 if s04b_20b == 1
egen spbja_monto = rowtotal(aux1 aux2) 

capture drop spbja_b /*BJA: controles bimestrales */
gen spbja_b = (s04d_27 ==1 ) if s04d_27 !=.

capture drop spbja_bmonto
gen spbja_bmonto = s04d_28b * 120   /* 125 bs por cada control post parto */
replace spbja_bmonto = 0 if spbja_bmonto ==. & spbja_b == 1
...
gen ysal1ben_5=(salmin*s06c_28a1)/12 if s06c_28a==1
...
*ASALARIADO
* Ingreso liquido en horario normal
fincome s06g_48b, by(s06f_45)						
recode  s06g_48a *=. if s06g_48a>=80000
capture drop ysal2net
gen ysal2net=.
replace ysal2net=s06g_48a*freq
...
recode s06g_49b1 *=. if s06g_49b1>=880000
gen ysal2esp_a=s06g_49b1/12

*2. Vivienda/Otros
recode s06g_49c1 *=. if s06g_49c1>=880000
gen ysal2esp_b=s06g_49c1/12
...
*Ingreso Total
fincome s06g_50b, by(s06f_45)
recode s06g_50a *=. if s06g_50a>=880000
capture drop yind2tot
gen yind2tot=.
replace  yind2tot=s06g_50a*freq


*Ingreso Neto
fincome s06g_52b, by(s06f_45)
recode s06g_52a *=. if s06g_52a>=880000
capture drop yind2net
gen yind2net=s06g_52a*freq
...
replace ysecjb=yind2net		if s06f_42==3 | s06f_42==5 | s06f_42==6
replace ysecjb=0			if s06f_42==7
...
for varlist s07a_04a s07a_04b s07a_04c: 				recode X *=. if X>=880000           /*ing.extraor*/ 
for varlist s07b_05aa s07b_05ba :		 			recode X *=. if X>=880000               /*tranf.o.h*/	
for varlist s07c_08a s07c_10:			 			recode X *=. if X>=880000 	//	        /*remesas*/		
                                                                                            				
*** Annualizing and converting to Bs.                                                       				
for varlist s07a_03a s07a_03b s07a_03c: 		replace X =X/12                             /*remesas*/		
for varlist s07a_04a s07a_04b s07a_04c:	 	replace X =X/12 
...
egen yothtot=rsum(s07a_04a s07a_04b s07a_04c) ,m
